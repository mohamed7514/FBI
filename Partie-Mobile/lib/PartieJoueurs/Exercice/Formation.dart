...
import 'package:app_parent/PartieJoueurs/Exercice/DetailExercice.dart';
import 'package:app_parent/PartieJoueurs/Exercice/DetailFormation.dart';
...
import 'package:app_parent/models/formation.dart';
...
import '../../HomeScreen/LicencePayement.dart';
import '../../HomeScreen/profil/profil.dart';
import '../../Login/SeConnecter.dart';
import '../../Outil/Outil.dart';
import '../../models/Evenemenet.dart';

class Formationjou extends StatefulWidget {
  const Formationjou({Key? key}) : super(key: key);

  @override
  State<Formationjou> createState() => _FormationjouState();
}

class _FormationjouState extends State<Formationjou> {
  List<Formation> futureData = [];
  void initState() {
    super.initState();
    getFormation();
  }
  ...
  getFormation() async {
    futureData = await FetchFormation();
    setState(() {
      print(futureData);
    });
  }
  ...
  Future<List<Formation>> FetchFormation() async {
    ...
    var request = await http.get(
      Uri.parse('http://172.16.130.41:8080/projet/formation'),
      headers: headers,
    );
    ...
    if (request.statusCode == 200) {
      ...
      midia = List<Formation>.from(l.map((model) => Formation.fromJson(model)));
      return midia;
    } else {
      return midia;
    }
  ...
  @override
  Widget Acualite(Formation midi) {
    ...
    return
      ...
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => DetailFormation(formation: midi)));
      },
      child: Container(
