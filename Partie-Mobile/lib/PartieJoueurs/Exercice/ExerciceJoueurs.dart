...
class ExerciceJou extends StatefulWidget {
  const ExerciceJou({Key? key}) : super(key: key);

  @override
  State<ExerciceJou> createState() => _ExerciceJouState();
}

class _ExerciceJouState extends State<ExerciceJou> {
  ...
  Future<List<Exercice>> FetchExercice() async {
    ...
    if (request.statusCode == 200) {
      ...
      midia = List<Exercice>.from(l.map((model) => Exercice.fromJson(model)));
      return midia;
    } else {
      return midia;
    }
    ...
    return
      ...
      onTap: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => DetailExerciceJou(Exercicee: midi)));
      },
      child: Container(
